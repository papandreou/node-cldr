            ...
            rule.rbnf.replace(/(?:([\<\>\=])(?:([\%\w\-]+)|([#,0.]+))?\1)|(?:\[([^\]]+)\])|([\x7f-\uffff:'\.\s\w\d\-]+)/gi, function ($0, specialChar, otherFormat, decimalFormat, optional, literal) {
                ...
                if (specialChar) {
                    ...
                    if (otherFormat) {
                        ...
                        var matchDecimalFormat = decimalFormat.match(/^#,###?0(?:[\.,](\#+))?$/);
                        if (matchDecimalFormat) {
                            var length = matchDecimalFormat[1] ? matchDecimalFormat[1].length : 0;
                            expressionAsts.push(['call', ['dot', ['name', 'this'], 'renderNumber'], [expr, ['num', length]]]);
                        } else {
                            throw new Error('Unsupported decimal format: ' + decimalFormat);
                        }
                    } else {
                        // Nothing:
                        if (specialChar === '>') {
                            ...
                            isSeenByRuleSetType[that.type] = true;
