...
Cldr.prototype = {
    ...
    get windowsZonesByMapZone() {
        if (!this._windowsZonesByMapZone) {
            ...
            xpath.select('/supplementalData/windowsZones/mapTimezones/mapZone', this.getDocument(Path.resolve(this.cldrPath, 'common', 'supplemental', 'windowsZones.xml'))).forEach(function (keyNode) {
                this._windowsZonesByMapZone.push({name: keyNode.getAttribute('type'), territory: keyNode.getAttribute('territory'), timezone: keyNode.getAttribute('other')});
            }, this);
        }
    ...
    resolveWindowsZonesByTimezone: function (timezone) {
        if (!timezone) {
            return;
        }
        var result = this.windowsZonesByMapZone.filter(function(element) { return element.timezone.toLowerCase() == timezone.toLowerCase(); });
        return result;
    },

    resolveWindowsZonesByName: function (name) {
        if (!name) {
            return;
        }
        var result = this.windowsZonesByMapZone.filter(function(element) { return element.name.toLowerCase() == name.toLowerCase(); });
        return result;
    },
