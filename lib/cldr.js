...
Cldr.prototype = {
    ...
    _extractPluralRules: function (localeId, cardinalOrOrdinal) {
        var that = this,
            ...
            pluralRules = new CldrPluralRuleSet();
    ...
    extractPluralRuleFunction: function (localeId, cardinalOrOrdinal) {
        return new Function('n', escodegen.generate({type: 'Program', body: this._extractPluralRuleAst(localeId, cardinalOrOrdinal)}));
    },
        ...
        while (typesToAdd.length > 0) {
            ...
            if (!(type in isAddedByType)) {
                ...
                if (cldrRbnfRuleSet) {
                    ...
                    rbnfFunctionByType[type] = new Function('n', escodegen.generate({type: 'Program', body: result.functionAst.body.body}));
    ...
    extractTerritoryInfo: function () {
        ...
        var territoryInfoByTerritoryId = {};
        finder('/supplementalData/territoryInfo/territory').forEach(function (territoryNode) {
            var territoryId = territoryNode.getAttribute('type');
        ...
        finder('/supplementalData/territoryInfo/territory/languagePopulation').forEach(function (languagePopulationNode) {
            ...
            var officialStatus = languagePopulationNode.getAttribute('officialStatus');
            if (officialStatus) {
                languageInfo.officialStatus = officialStatus;
            }
            var writingPercent = languagePopulationNode.getAttribute('writingPercent');
            if (writingPercent) {
