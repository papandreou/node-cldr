...
Cldr.prototype = {
    ...
    getPrioritizedDocumentsForLocale: function (localeId, type) {
        ...
        var fileNames = expandLocaleIdToPrioritizedList(localeId).concat('root').map(function (subLocaleId, i) {
            var fileName = that.fileNamesByTypeAndNormalizedLocaleId[type][normalizeLocaleId(subLocaleId)];
            // Bail out if there's no data for the top level locale, but allow sublocale extracting eg. da_DK when only da.xml exists:
            if (!fileName && i > subLocaleIds.length - 2) {
                throw new Error('No data for locale id: ' + subLocaleId);
            }
            return fileName;
        });
