...
Cldr.prototype = {
  ...
  extractDateTimePatterns(localeId, calendarId) {
    ...
    finder(
      ...
      const dateTimeFormatLengthType =
        dateTimeFormatNode.parentNode.getAttribute('type');
  ...
  extractDateIntervalFormats(localeId, calendarId) {
    ...
    finder(
      ...
      for (let i = 0; i < intervalFormatItemNode.childNodes.length; i += 1) {
        ...
        const greatestDifferenceIdAttribute =
          greatestDifferenceNode.getAttribute('id');
  ...
  extractListPatterns(localeId) {
    ...
    finder('/ldml/listPatterns/listPattern/listPatternPart').forEach(
      (listPatternPartNode) => {
        const listPatternTypeAttribute =
          listPatternPartNode.parentNode.getAttribute('type');
    ...
    xpath
      ...
      .forEach((numberingSystemNode) => {
        ...
        if (numberingSystemNode.getAttribute('type') === 'numeric') {
          digitsByNumberSystemId[numberSystemId] = numberingSystemNode
            .getAttribute('digits')
            .split(/(?:)/u);
        } else {
          // type='algorithmic'
  ...
  extractTerritoryInfo() {
    ...
    for (const languagePopulationNode of finder(
      ...
      const territoryId =
        languagePopulationNode.parentNode.getAttribute('type');
      const languageInfo = {
      ...
      const officialStatus =
        languagePopulationNode.getAttribute('officialStatus');
      if (officialStatus) {
        languageInfo.officialStatus = officialStatus;
      }
      const writingPercent =
        languagePopulationNode.getAttribute('writingPercent');
      if (writingPercent) {
  ...
  extractTTSCharacterLabels(localeId) {
    this.checkValidLocaleId(localeId);
    const finder = this.createFinder(
    ...
    finder('/ldml/annotations/annotation[@type="tts"]').forEach((node) => {
  ...
  extractDerivedTTSCharacterLabels(localeId) {
    this.checkValidLocaleId(localeId);
    const finder = this.createFinder(
  ...
  extractAllTTSCharacterLabels(localeId) {
    return {
      ...this.extractDerivedTTSCharacterLabels(localeId),
      ...this.extractTTSCharacterLabels(localeId),
    };
  },
};
