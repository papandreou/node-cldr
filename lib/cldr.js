...
Cldr.prototype = {
  ...
  extractEraNames(localeId, calendarId) {
    ...
    ['eraNames', 'eraAbbr'].forEach(eraType => {
      const typeInOutput = { eraNames: 'wide', eraAbbr: 'abbreviated' }[eraType];
      finder(
  ...
  extractRbnfFunctionByType(localeId, types) {
    ...
    finder('/ldml/rbnf/rulesetGrouping/ruleset/rbnfrule').forEach(rbnfRuleNode => {
  ...
  extractTerritories() {
    ...
    finder('/supplementalData/codeMappings/territoryCodes').forEach(territoryCodeNode => {
  ...
  extractTerritoryInfo() {
    ...
    const territoryInfoByTerritoryId = {};
    finder('/supplementalData/territoryInfo/territory').forEach(territoryNode => {
      const territoryId = territoryNode.getAttribute('type');
  ...
  extractTerritoryContainmentGroups() {
    ...
    finder('/supplementalData/territoryContainment/group').forEach(groupNode => {
  ...
  extractLanguageSupplementalMetadata() {
    ...
    finder('/supplementalData/metadata/alias/languageAlias').forEach(languageAliasNode => {
