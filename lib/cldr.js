    ...
    extractWorldInfo: function () {
        ...
        finder("/supplementalData/territoryContainment/group").forEach(function (groupNode) {
            ...
            if (!isSeenByType[type] && status !== 'deprecated') {
                ...
                if (type in countryInfoByTerritoryId) {
                    countryInfoByTerritoryId[type].isSubdivided = true;
                }
                ...
                contains.forEach(function (id) {
                    ...
                    if (/^\d+$/.test(id)) {
                        ...
                        containedTerritoriesByNumericTerritoryId[numeric] = true;
        ...
        Object.keys(countryInfoByTerritoryId).forEach(function (territoryId) {
            var numeric;
            if (territoryId in numericTerritoryIdByAlpha2Code) {
                numeric = numericTerritoryIdByAlpha2Code[territoryId];
            }

            if (numeric && !containedTerritoriesByNumericTerritoryId[numeric]) {
                delete alpha2CodeByNumericTerritoryId[numeric];
            }

            if (!containedTerritoriesByAlpha2TerritoryId[territoryId]) {
                delete countryInfoByTerritoryId[territoryId];
            }
        });
