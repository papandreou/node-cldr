...
Cldr.prototype = {
    ...
    extractWorldInfo: function () {
        ...
        var countryInfosByTerritoryId = {};

        var containedTerritoriesByTerritoryId = {};
        ...
        finder("/supplementalData/codeMappings/territoryCodes").forEach(function (territoryCodeNode) {
            ...
            var numeric = territoryCodeNode.getAttribute('numeric');
            var alpha3 = territoryCodeNode.getAttribute('alpha3');
            
            countryInfosByTerritoryId[type] = [{
                numericCode: numeric,
                alpha3Code: alpha3,
                alpha2Code: type,
                isIndependent: false,
            ...
            if (numeric) {
                alpha2CodeByNumericTerritoryId[numeric] = type;
                numericTerritoryIdByAlpha2Code[type] = numeric;

                countryInfosByTerritoryId[type][0].isIndependent = true;
            } else {
                countryInfosByTerritoryId[type][0].isSubdivision = true;
            }
        });
        ...
        finder("/supplementalData/territoryContainment/group").forEach(function (groupNode) {
            ...
            var status = groupNode.getAttribute('status');
            if (!isSeenByType[type] && status !== 'deprecated') {
                ...
                contains.forEach(function (id) {
                    ...
                    if (/^\d+$/.test(id)) {
                        ...
                        containedTerritoriesByTerritoryId[id] = true;
                    }

                    if (numeric !== undefined && !(numeric in parentRegionIdByTerritoryId) && /^\d+$/.test(type)) {
        ...
        Object.keys(countryInfosByTerritoryId).forEach(function (territoryId) {
            if (!containedTerritoriesByTerritoryId[territoryId]) {
                delete countryInfosByTerritoryId[territoryId];
            }
        });

        return {
            alpha2CodeByNumericTerritoryId: alpha2CodeByNumericTerritoryId,
            countryInfosByTerritoryId: countryInfosByTerritoryId,
