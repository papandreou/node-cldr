...
Cldr.prototype = {
    ...
    extractFields: function (localeId) {
        ...
        finder("/ldml/dates/fields/field/relativeTime/relativeTimePattern").forEach(function (relativeTimePatternNode) {
            var relativeTimeNode = relativeTimePatternNode.parent(),
                fieldName = relativeTimeNode.parent().attr('type').value(),
                type = relativeTimeNode.attr('type').value(),
                count = relativeTimePatternNode.attr('count').value();
            fields = fields || {};
            fields[fieldName] = fields[fieldName] || {};
            ...
            fields[fieldName].relativeTime[type][count] = fields[fieldName].relativeTime[type][count] || relativeTimePatternNode.text()
        });
        return fields;
    },
    ...
    extractUnitPatterns: function (localeId) {
        ...
        finder("/ldml/units/unitLength/unit/unitPattern").forEach(function (unitPatternNode) {
            var unitNode = unitPatternNode.parent(),
                unitLength = unitNode.parent().attr('type').value(),
                unitId = normalizeProperty(unitNode.attr('type').value());
            ...
            var count = unitPatternNode.attr('count').value();
            unitPatterns[unitLength].unit[unitId][count] = unitPatterns[unitLength].unit[unitId][count] || unitPatternNode.text();
        });
        finder("/ldml/units/unitLength/compoundUnit/compoundUnitPattern").forEach(function (compoundUnitPatternNode) {
            var compoundUnitNode = compoundUnitPatternNode.parent(),
                unitLength = compoundUnitNode.parent().attr('type').value(),
                compoundUnitId = compoundUnitNode.attr('type').value();

            unitPatterns[unitLength].compoundUnit = unitPatterns[unitLength].compoundUnit || {};
            unitPatterns[unitLength].compoundUnit[compoundUnitId] = compoundUnitPatternNode.text();
        });
        return unitPatterns;
    },
    ...
    extractListPatterns: function (localeId) {
        ...
        finder("/ldml/listPatterns/listPattern/listPatternPart").forEach(function (listPatternPartNode) {
            var listPatternTypeAttribute = listPatternPartNode.parent().attr('type'),
                type = listPatternTypeAttribute ? normalizeProperty(listPatternTypeAttribute.value()) : 'default',
                part = listPatternPartNode.attr('type').value();
            listPatterns[type] = listPatterns[type] || {};
            listPatterns[type][part] = listPatterns[type][part] || listPatternPartNode.text();
        });
        return listPatterns;
    },
    ...
    extractLayout: function (localeId) {
        ...
        finder("/ldml/layout/*/*").forEach(function (leafNode) {
            var type = leafNode.name(),
                parentType = leafNode.parent().name();
            layout[parentType] = layout[parentType] || {};
            layout[parentType][type] = layout[parentType][type] || leafNode.text();
        });
        return layout;
    }
};
