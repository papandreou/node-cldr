...
one.locale = {
    ...
    getCodeFragmentForDateField: (function () {
        var codeFragmentsByFormatChar = {
            G: ['one.locale.eraNames.abbreviated[{date}.getFullYear() > 0 ? 1 : 0]'], // Era designator
            y: ['one.locale.leftPad({date}.getFullYear(), 4, "0")'],
            //Y: [], // Week of Year
            //u: [], // Extended year
            Q: ['one.locale.leftPad(Math.floor({date}.getMonth()/4), 2, "0")', '*', 'one.locale.quarterNames.format.abbreviated[Math.floor({date}.getMonth()/4)]', 'one.locale.quarterNames.format.wide[Math.floor({date}.getMonth()/4)]'], // Quarter
            //q: [], // Stand alone quarter
            M: ['({date}.getMonth() + 1)', 'one.locale.leftPad({date}.getMonth() + 1, 2, "0")', 'one.locale.monthNames.format.abbreviated[{date}.getMonth()]', 'one.locale.monthNames.format.wide[{date}.getMonth()]'],
            L: ['({date}.getMonth() + 1)', 'one.locale.leftPad({date}.getMonth() + 1, 2, "0")', 'Date.getShortMonthName({date}.getMonth())', 'Date.monthNames[{date}.getMonth()]'],
            //w: [], // Week of year
            //W: [], // Week of month
            d: ['{date}.getDate()', 'one.locale.leftPad({date}.getDate(), 2, "0")'],
            //D: [], // Day of year
            //F: [], // Day of week in month
            //g: [], // Modified Julian day
            E: ['one.locale.dayNames.format.abbreviated[{date}.getDay()]', '*', '*', 'one.locale.dayNames.format.wide[{date}.getDay()]'],
            //e: [], // Local day of week
            //c: [], // Stand alone day of week
            a: ['({date}.getHours() < 12 ? "am" : "pm")'],
            h: ['(({date}.getHours() % 12) ? {date}.getHours() % 12 : 12)'],
            H: ['one.locale.leftPad({date}.getHours(), 2, "0")'],
            //k: [], // Hour in day (1-24)
            //K: [], // Hour in am/pm (0-11)
            m: ['one.locale.leftPad({date}.getMinutes(), 2, "0")'],
            s: ['one.locale.leftPad({date}.getSeconds(), 2, "0")']
            //S: [], // Millisecond
    ...
    makeDateRendererSource: function (sourceVariableNameOrExpression, format) {
        ...
        this.tokenizeDateFormat(format).forEach(function (token) {
            if (token.type === 'text') {
                ...
                // token.type === 'field'
                var codeFragment = one.locale.getCodeFragmentForDateField(token.value, sourceVariableNameOrExpression);
                if (codeFragment) {
    ...
    makeDateRenderer: function (formatId) {
        ...
        if (formatId === 'hv') {
            ...
            if (!format) {
                throw new Error('one.locale.makeDateRenderer: Cannot find date format: ' + formatId);
            }
        }
        ...
        if (greatestDifferences) {
            ...
            if (datePartOnly && matchFormatId) {
                ...
                return function (dateInterval) {
                    return one.locale.renderDateInterval(dateInterval, dateFormatId);
                };
            } else {
                ...
                if (dateFormat) {
                    ...
                    throw new Error("one.locale.renderDateInterval: No usable date interval format found for " + formatId);
                }
            }
        }
    },
    ...
    makeDateIntervalRendererFromFormatString: function (format) {
        ...
        this.tokenizeDateFormat(format).forEach(function (token) {
            if (token.type === 'text') {
                ...
                // token.type === 'field'
                expressions.push(one.locale.getCodeFragmentForDateField(token.value, 'dateInterval.' + (seenFields[token.value[0]] ? 'end' : 'start')));
 ...
 *   one.locale.renderDateInterval(firstTenDaysOfJune, "yMMd"); // "06/1/2010-06/10/2010" (en_US)
 *   one.locale.renderDateInterval(firstTenDaysOfJune, "yMMMMd"); // "June 1-10, 2010" (en_US)
 * </code></pre>
 ...
 * @name one.locale.renderDateInterval
 * @function
 */
 ...
 * Example invocation:
 * <pre><code>
 *   var renderer = one.locale.getDateIntervalRenderer("yMMMM"),
 ...
 * @return {Function} The date or date-time interval renderer,
 * Object{start: Date, end: Date} => String.
 * @name one.locale.getDateIntervalRenderer
 * @function
 */
...
// methods.
// The renderer functions themselves are cached in one.locale.renderers.
